...
class App extends baseComponent {
	...
	openMap = ()=>{
		...
		let city = this.state.form.city.value;
		...
		let addressValue = (city + form.customAddress.value).replace(reg,'');
		let cityValue = city.split(' ')[1].replace(reg,'');
		this.setState({map_show:true});


		if (!window.BMap) {
			const initMapFirst = ()=>{
				this.refs.map.initMap(addressValue , cityValue);
			}

			window.initMapFirst = initMapFirst;
			window.onload = this.refs.map.asyncMap();
		}else{
			this.refs.map.initMap(addressValue , cityValue);
		}
	}
	...
	sumbit = e=>{
		...
		let valid = this.validate(this.state.form);
			...
			if (!valid) {
				let msg = this.errorMessage.length ===1 ? this.errorMessage[0] : '亲, 你有些信息没有填完整';
					...
					<section>
						<div className="input-group">
							...
							<FieldItem label="门店地址">
								<Input name="customAddress" onChange={this.handleChange} placeholder="例如：广州分店"></Input>
							</FieldItem>
